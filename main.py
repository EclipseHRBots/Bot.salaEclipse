              ...
              if len(split) == 2:
                  ...
                  try:
                      for u in users:
                          u_give = str("@") + str((u.username).lower())
                          if str((u_give).lower()).strip() == str(name).strip():
                              await self.highrise.teleport(u.id,Position(x=15.0, y=4.5, z=5.7, facing='FrontRight')) 
                              break
                  except:
                      pass
              else:
                  await self.highrise.teleport(user.id,Position(x=15.0, y=4.5, z=5.7, facing='FrontRight'))
