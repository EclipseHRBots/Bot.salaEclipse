...
moderators = ["iMooseMoo","Geomary37","miaa552","Nezux","Miojindoce","_DOUTORA_","zlFrizz_"]
...
class Bot(BaseBot):
    ...
    async def on_user_join(self, user: User, position: Position | AnchorPosition) -> None:
        ...
        await self.highrise.send_whisper(user.id,f"Bienvenido a ECLIPSE {user.username} si necesitas ayuda usa !help  ")
              ...
              if len(split) == 2:
                  ...
                  try:
                      for u in users:
                          u_give = str("@") + str((u.username).lower())
                          if str((u_give).lower()).strip() == str(name).strip():
                              await self.highrise.teleport(u.id,Position(x=14.5, y=13.5, z=3.0, facing='FrontRight')) 
                              break
                  except:
                      pass
              else:
                  await self.highrise.teleport(user.id,Position(x=14.5, y=13.5, z=3.0, facing='FrontRight'))
