...
moderators = ["","","Hdaniel.07 ","Nezux","ElCordobez","Hdaniel.07",""]
...
class Bot(BaseBot):
    async def on_start(self, session_metadata: SessionMetadata) -> None:
        print("working")
        await self.highrise.walk_to(Position(17 , 0 , 25 , "FrontLeft"))
    async def on_user_join(self, user: User, position: Position | AnchorPosition) -> None:
        print(f"{user.username} entrou na sala")   
        await self.highrise.send_whisper(user.id,f"Bienvenido a Fenix CLUB ESpero que disfrute de la sala, y de la buena musica con HR Caserio {user.username} si necesitas ayuda usa !help  ")
        ...
        if message.startswith("!dj"):
          if user.username in moderators:
              ...
              if len(split) == 2:
                  ...
                  await self.highrise.teleport(user.id,Position(x=5, y=3, z=13.0, facing='FrontRight'))
